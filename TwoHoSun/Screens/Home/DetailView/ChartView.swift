...
extension ChartView {
                    ...
                    GeometryReader { geo in
                        HStack(spacing: 0) {
                            Rectangle()
                                ...
                                .frame(width: max(0, geo.size.width * Double(data.agree) / Double(total)))
                            Rectangle()
    ...
    private func gradeChartContent(label: String, data: AgreeCount) -> some View {
            HStack {
                HStack {
                    ...
                    Rectangle()
                        .foregroundStyle(.cyan)
                        .frame(width: max(0, 90 * Double(data.agree) / Double(total)), height: 15)
                }
                Text(label)
                ...
                HStack {
                    Rectangle()
                        .foregroundStyle(.green)
                        .frame(width: max(0, 90 * Double(data.disagree) / Double(total)), height: 15)
    ...
    private func regionChartContent(label: String, data: AgreeCount) -> some View {
        VStack {
            ...
            Rectangle()
                .foregroundStyle(.cyan)
                .frame(width: 15, height: max(0, 40 * Double(data.agree) / Double(total)))
            Text(label)
                .font(.system(size: 14, weight: .medium))
            Rectangle()
                .foregroundStyle(.green)
                .frame(width: 15, height: max(0, 40 * Double(data.disagree) / Double(total)))
            Text(String(format: "%.1f%%", Double(data.disagree) / Double(total) * 100))
                .foregroundStyle(.gray)
                .font(.system(size: 12, weight: .medium))
                .fixedSize()
